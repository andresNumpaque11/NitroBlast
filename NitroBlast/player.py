...
class Player:
    ...
    def move(self,dx, dy, grid, enemys, power_ups):
        ...
        for x in enemys:
            if x == self:
                ...
                map[int(self.pos_x / Player.TILE_SIZE)][int(self.pos_y / Player.TILE_SIZE)]=2
    ...
    def plant_bomb(self, map):
        b = Bomb(
            self.range,
            round(self.pos_x / Player.TILE_SIZE),
            round(self.pos_y / Player.TILE_SIZE),
            map,
            self,
        )
        return b
    ...
    def check_death(self, exp):
        for e in exp:
            for s in e.sectors:
                if(
                    int(self.pos_x / Player.TILE_SIZE) == s[0]
                    and int(self.pos_y / Player.TILE_SIZE) == s[1]
                ):
                    self.life= False
     
    def consume_power_up(self, power_up, power_ups):
        if power_up.type == Power_Up_Type.BOMB:
            self.bomb_limit += 1
        elif power_up.type == Power_Up_Type.FIRE:
            self.range += 1
